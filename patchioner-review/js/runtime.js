  ...
  ready(function () {
    ...
    if (isPreviewMode) {
      ...
      window.addEventListener('message', function(e) {
        ...
        if (e.data.type === 'preview-goto') {
          ...
          link.href = previewThemeBase + 'patchioner.css';
    ...
    if (!overview) {
      ...
      slides.forEach((s, i) => {
        ...
        mini.style.top = '0';
        mini.style.left = '0';
        mini.style.width = '1600px';
        mini.style.height = '900px';
        ...
        clone.className = s.className + ' is-active'; // force active styles
        clone.style.position = 'absolute';
        clone.style.inset = '0';
        clone.style.transform = 'none';
        clone.style.opacity = '1';
        clone.style.padding = '62px 92px 74px'; // ensure padding is kept
    ...
    function toggleOverview(force){
      ...
      if (isOpen) {
        requestAnimationFrame(() => {
          ...
          if (thumbs.length) {
            const scale = thumbs[0].clientWidth / 1600;
     ...
     *   SCRIPT   — large speaker notes (발표 대본)
    ...
    function openPresenterWindow() {
      ...
      presenterWin = window.open(deckUrl + '?presenter=1#/' + (idx + 1), 'html-ppt-presenter', 'width=1280,height=820,menubar=no,toolbar=no');
      if (!presenterWin) {
        alert('발표자 화면을 열려면 팝업을 허용해주세요.');
        return;
      }
    }
...
<div class="hint-bar">
  ...
  <span><kbd>R</kbd> 타이머 초기화</span>
  <span><kbd>Esc</kbd> 닫기</span>
  <span style="color:#6e7681">카드 제목을 드래그하여 이동 · 오른쪽 아래를 드래그하여 크기 조절</span>
  <button class="reset-layout" id="reset-layout">배치 초기화</button>
</div>
...
(function(){
  ...
  function rescaleIframe(iframe) {
    ...
    if (!cw || !ch) return;
    var s = Math.min(cw / 1600, ch / 900);
    iframe.style.transform = 'scale(' + s + ')';
    /* Center the scaled iframe in the body */
    var sw = 1600 * s, sh = 900 * s;
  ...
  document.getElementById('reset-layout').addEventListener('click', function(){
    if (confirm('발표자 화면을 기본 배치로 복원할까요?')) {
  ...
  timerCount.textContent = (idx + 1) + ' / ' + total;
    ...
    if (new URLSearchParams(location.search).has('presenter')) {
      const deckUrl = location.protocol + '//' + location.host + location.pathname;
      const slideMeta = slides.map(s => ({title:s.dataset.title, notes:s.querySelector('.notes')?.innerHTML || ''}));
      const presenterHTML = buildPresenterHTML(deckUrl, slideMeta, total, idx, CHANNEL_NAME, 'academic-clean');
      document.open(); document.write(presenterHTML); document.close();
    }
  });
})();
